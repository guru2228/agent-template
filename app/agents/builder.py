    ...
    def with_model(
        ...
        try:
            if model_id:
                ...
                self._model = ModelFactory.get_model(model_id, **kwargs)
            elif provider:
                # Use provider default
                self._model = ModelFactory.get(provider, **kwargs)
            else:
                raise ValueError("Either provider or model_id must be specified")
        except Exception as e:
            discovery_logger.error(f"Failed to configure model {provider or model_id}: {e}")
            raise ValueError(f"Failed to configure model {provider or model_id}: {e}")
