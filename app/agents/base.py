...
class DiscoveryPattern(IntEnum):
    ...
    Only YAML-based discovery is supported.
    """

    YAML = 1       # YAML-based agent definitions
...
class AgentDefinition:
    ...
    def __post_init__(self):
        ...
        # Validate source file exists (skip for certain sources)
        skip_validation_sources = [
            "unknown",  # Unknown source (e.g., decorator pattern)
        ]
        
        # Skip validation for environment variable sources
        is_env_source = self.source_file.startswith(("env:", "TEST_", "AGENTS_", "YAML_"))
        
        should_skip_validation = (
            self.source_file in skip_validation_sources or 
            is_env_source or
            self.module_path.startswith("env:")
        )
        
        if not should_skip_validation and not Path(self.source_file).exists():
